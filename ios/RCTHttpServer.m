...
- (void)initResponseReceivedFor:(GCDWebServer *)server forType:(NSString*)type {
    [server addDefaultHandlerForMethod:type
        ...
        NSDictionary *postBody = ([(GCDWebServerURLEncodedFormRequest*)request arguments]) ? ([(GCDWebServerURLEncodedFormRequest*)request arguments]) : @{};
                  ...
                  body: (NSString *) body)
{
    ...
    for (id key in responseHeaders) {
        [requestResponse setValue:[responseHeaders objectForKey:key] forAdditionalHeader:key];
    }
